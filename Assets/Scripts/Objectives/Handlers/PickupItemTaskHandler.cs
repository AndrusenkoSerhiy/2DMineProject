...
namespace Objectives.Handlers {
  public class PickupItemTaskHandler : IObjectiveTaskHandler {
    public ObjectiveTaskType Type => ObjectiveTaskType.PickupItem;

    public bool IsTaskSatisfied(ObjectiveTaskData taskData, object context, int currentProgress,
      out int progressToAdd) {
      progressToAdd = 0;

      if (taskData is not PickupItemTaskData data || context is not (ItemObject item, int amount)) {
        return false;
      }
